...
# R8 full mode (enabled in gradle.properties) strips more aggressively than
# compatibility mode, including generic signatures, annotations, inner-class
# metadata and Kotlin reflection data. Reanimated/worklets and several Expo
# modules rely on reflection and annotations, so these attribute keeps are
# load-bearing.
...
const GRADLE_PROPERTIES: Record<string, string> = {
  ...
  // R8 full mode has been the AGP default since 8.0; pin it explicitly so a
  // future template default can't silently weaken release optimization.
...
const withMaidBuildGradle: ConfigPlugin = (config) => {
  ...
  return withAppBuildGradle(config, (config) => {
    ...
    // Use the optimizing ProGuard defaults: `proguard-android-optimize.txt`
    // enables R8's extra optimization passes (-optimizations, inlining, etc.)
    // over the plain `proguard-android.txt` that ships with the Expo template.
