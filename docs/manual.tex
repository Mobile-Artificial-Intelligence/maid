...
Maid organises your conversations as \textbf{chats}. Each chat is an independent thread with its own message history. Chats are stored locally on your device and persist between sessions. Because messages are arranged as a tree rather than a flat list, Maid supports branching and editing without losing any previous responses (see Sections~\ref{sec:branching} and~\ref{sec:editing}).

\subsection{Creating a New Chat}

\begin{enumerate}
    \item Open the drawer by tapping the \textbf{menu} icon in the top-left corner of the screen, or by swiping in from the left edge.
    \item In the \textbf{Chats} section of the drawer, tap the \textbf{+} button in the section header.
    \item A new empty chat titled \emph{New Chat} is created and immediately becomes the active chat.
\end{enumerate}

\subsection{Switching Chats}

\begin{enumerate}
    \item Open the drawer.
    \item Tap any chat name in the list. The selected chat becomes active and its message history is displayed.
\end{enumerate}

To rename, export, or delete a chat, \textbf{long-press} its name in the drawer to open a context menu:

\begin{itemize}[noitemsep]
    \item \textbf{Rename} — A text field appears in place of the chat name. Type the new title and press Return to confirm, or wait for the field to time out.
    \item \textbf{Export} — Saves the entire chat (including all branches) as a JSON file that can be re-imported later.
    \item \textbf{Delete} — Permanently removes the chat and all its messages.
\end{itemize}

% ─────────────────────────────────────────────────────────────────────────────
\section{Branching Chats}
\label{sec:branching}

\subsection{What is Branching}

Every chat in Maid is stored as a message tree rather than a flat list. A single user message can have multiple AI responses as \emph{branches}, and each of those responses can themselves be branched further. Branching lets you explore different model outputs from the same point in a conversation without losing any of them.

\subsection{Navigating Branches}

When a message has more than one branch, left and right arrow buttons appear next to it along with a counter showing your current position, for example \textbf{2 / 3} (branch 2 of 3).

\begin{itemize}[noitemsep]
    \item Tap the \textbf{left arrow} to move to the previous branch.
    \item Tap the \textbf{right arrow} to move to the next branch.
\end{itemize}

The conversation displayed below that point always follows the currently selected branch. Switching branches does not delete any alternatives — all branches are retained.

% ─────────────────────────────────────────────────────────────────────────────
\section{Editing Messages}
\label{sec:editing}

You can edit any of your own (user) messages at any point in a conversation.

\begin{enumerate}
    \item Tap the \textbf{pencil} icon that appears in the top-right corner of a user message.
    \item An inline text editor opens with the current message content pre-filled. Make your changes.
    \item Tap the \textbf{check mark} to confirm. Maid creates a new branch from the edited message and automatically requests a fresh AI response.
    \item Tap the \textbf{X} to cancel without making any changes.
\end{enumerate}

The original message and its response are preserved as a separate branch. Use the branch navigation arrows to switch between the original and edited versions.

% ─────────────────────────────────────────────────────────────────────────────
\section{Regenerating Responses}

If you are not satisfied with an AI response you can ask the model to try again.

\begin{enumerate}
    \item Tap the \textbf{reload} icon that appears in the top-right corner of an assistant message.
    \item Maid creates a new branch under the same user message and sends the conversation to the model again.
    \item The new response streams in and becomes the active branch.
\end{enumerate}

The original response is not deleted. Use the branch navigation arrows (see Section~\ref{sec:branching}) to compare the original and regenerated responses.

% ─────────────────────────────────────────────────────────────────────────────
\section{Dictation}

Maid supports voice input so you can speak your messages instead of typing them.

\subsection{Starting Dictation}

When the text input field is empty and no response is being generated, a \textbf{microphone} icon appears on the right side of the input bar. Tap it to begin recording. On first use, Maid will request microphone permission.

\subsection{During Recording}

Speak your message naturally. Maid uses your device's speech recognition engine to transcribe what you say and inserts the transcript into the input field. Recording stops automatically after a period of silence. The microphone icon changes to a \textbf{mic-off} icon while recording is active — tap it to stop recording early.

\subsection{After Recording}

The transcribed text appears in the input field. You can review and edit it before sending, or tap the \textbf{send} button to submit it immediately.

\textbf{Note:} Dictation requires an internet connection on most devices, as speech recognition is handled by the operating system's cloud service.

% ─────────────────────────────────────────────────────────────────────────────
\section{Stopping Generation}

To interrupt an in-progress AI response, tap the \textbf{stop} button (a filled square icon) that replaces the send button in the input bar while the model is generating. Generation halts immediately and whatever text has already been produced is kept in the chat. You can then:

\begin{itemize}[noitemsep]
    \item Continue the conversation by sending a follow-up message.
    \item Regenerate the response (see the previous section).
    \item Edit the preceding user message and branch in a different direction.
\end{itemize}
