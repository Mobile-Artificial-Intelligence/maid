...
Maid (Mobile Artificial Intelligence Distribution) is a free and open-source application for interfacing with AI language models on Android. It supports both fully local inference via \texttt{llama.cpp} and remote inference through the APIs of Anthropic, DeepSeek, Mistral, Novita, Ollama, llmman, and OpenAI.
...
\toprule
\textbf{Provider} & \textbf{API Key} & \textbf{Custom Base URL} & \textbf{Custom Headers} & \textbf{Local} & \textbf{Vision} \\
\midrule
Llama    & No  & No  & No  & Yes & Yes (requires projector) \\
Ollama   & No  & Yes & Yes & Yes & Yes (model-dependent) \\
llmman   & No  & Yes & Yes & Yes & Yes (model-dependent) \\
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{llmman} — Local or self-hosted server (Ollama-compatible API)
...
\midrule
Qwen 3.5 0.8B              & 0.8B & UD-IQ2\_M through BF16 \\
Qwen 3.5 2B                & 2.0B & UD-IQ2\_M through BF16 \\
Qwen 3.5 4B                & 4.0B & UD-IQ2\_M through BF16 \\
LFM 2.5 1.2B Thinking      & 1.2B & Q4\_0, Q4\_K\_M, Q5\_K\_M, Q6\_K, Q8\_0, BF16, F16 \\
LFM 2.5 1.2B Instruct      & 1.2B & Q4\_0, Q4\_K\_M, Q5\_K\_M, Q6\_K, Q8\_0, BF16, F16 \\
LFM 2.5 VL 1.6B            & 1.6B & Q8\_0, BF16, F16 \\
...
\subsection{Vision Support (Multimodal)}
\label{sec:llama-vision}

The Llama provider supports image input for vision-capable models. To enable this, you must load a \textbf{multimodal projector} file alongside the base model.

\subsubsection{What is a Projector?}

A multimodal projector (also called a clip or vision adapter) is a small auxiliary model file that bridges the image encoder and the language model. It is distributed separately from the base GGUF model and typically has the file extension \texttt{.mmproj} or \texttt{.gguf}.

\subsubsection{Loading a Projector File}

\begin{enumerate}
    \item Transfer the projector file to your device.
    \item In Maid, go to \textbf{Settings} and select \textbf{Llama} as the API.
    \item Tap \textbf{Add Projector File} and select the file using the file picker. Both \texttt{.mmproj} and \texttt{.gguf} files are accepted.
    \item The projector is automatically linked to the currently active model.
\end{enumerate}
...
\subsubsection{Projector Matching Rules}

The projector is used when:
\begin{itemize}[noitemsep]
    \item The selected projector key matches the selected model key exactly, \textbf{or}
    \item The model was loaded from a local file (model key ends in \texttt{(local)}).
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Projector} — Optional multimodal projector file for vision support (see Section~\ref{sec:llama-vision}).
...
\subsection{Find Ollama}

Instead of entering the Base URL manually, tap the \textbf{Find Ollama} button to have Maid scan your local network automatically. Maid determines the device's current IP address, derives the subnet, and simultaneously probes every host on that subnet at port \texttt{11434}. The first responding host is set as the Base URL. An alert is shown if no Ollama instance is found.
...
\begin{enumerate}
    ...
    \item Enter the \textbf{Base URL} of your Ollama server manually, or tap \textbf{Find Ollama} to detect it automatically (see above). Example URL:
\begin{lstlisting}
...
Vision support in Ollama is \textbf{automatic} — Maid queries the selected model's capabilities when you choose a model. If the model reports vision support (e.g.\ \texttt{llava}, \texttt{bakllava}, \texttt{moondream}, \texttt{llama3.2-vision}), the image attachment button is enabled in the prompt input bar. No additional configuration is required.

To use a vision-capable model, pull it with Ollama on the host machine and then select it from the \textbf{Model} dropdown in Maid:
...
\href{https://github.com/llmmanorg/llmman}{llmman} is a local model runner that serves the Ollama API (alongside OpenAI- and Anthropic-compatible ones) on port \texttt{17434}. The \textbf{llmman} provider in Maid behaves exactly like the Ollama provider described above, with the Base URL defaulting to \texttt{http://localhost:17434} and \textbf{Find llmman} probing port \texttt{17434}. Install it, start the server and pull a model with:
...
To accept connections from other devices, set \texttt{LLMMAN\_HOST=0.0.0.0} before running \texttt{llmman serve}.
...
\subsection{Attaching Images}
\label{sec:images}

When the active provider and model support vision (currently \textbf{Llama} with a projector loaded and \textbf{Ollama} with a vision-capable model), an \textbf{image} icon appears to the left of the action button in the prompt input bar. The button is disabled when vision is not available.

To attach images:

\begin{enumerate}[noitemsep]
    \item Tap the \textbf{image} icon. Maid will request photo library access on first use.
    \item Select one or more images from your photo library.
    \item The selected images appear as thumbnails in a horizontal strip above the input field. Tap the \textbf{×} on any thumbnail to remove it before sending.
    \item Type your message (optional) and tap \textbf{Send}. The images are included with your message and passed to the model alongside the text.
\end{enumerate}
